...
{
    public partial class YetkiliBilgileri : Form
    {
        ...
        private void YetkiliBilgileri_Load(object sender, EventArgs e)
        {
            con.Close();
            con.Open();
            MySqlCommand cmd = new MySqlCommand("SELECT *FROM uyeler WHERE uye_ogrencino=@gkadi", con);
            cmd.Parameters.AddWithValue("@gkadi", YetkiliGiris.gkadi);
            ...
            if (kr.Read())
            {
                ...
                con.Close();
                yetad.Text = _ad;
                yetsoyad.Text = _soyad;
                tarih.Text = _tarih;
                yetcc.Text = _tcno;
                ogrno.Text = YetkiliGiris.gkadi;
                telno.Text = _telno;
                adres.Text = _adres;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            con.Open();
            MySqlCommand dmd = new MySqlCommand("UPDATE uyeler SET uye_tel='" + _telno + "' WHERE uye_ogrencino='" + YetkiliGiris.gkadi + "'", con);
            dmd.ExecuteNonQuery();
            con.Close();
            con.Open();
            MySqlCommand amd = new MySqlCommand("UPDATE uyeler SET uye_adres='" + _adres + "' WHERE uye_ogrencino='" + YetkiliGiris.gkadi + "'", con);
            amd.ExecuteNonQuery();
            con.Close();
            con.Open();
            MySqlCommand omd = new MySqlCommand("UPDATE uyeler SET uye_ogrenciposta='" + _ogrenci_posta + "' WHERE uye_ogrencino='" + YetkiliGiris.gkadi + "'", con);
            omd.ExecuteNonQuery();
            con.Close();
            con.Open();
            MySqlCommand kmd = new MySqlCommand("UPDATE uyeler SET uye_kisiselposta='" + _kisisel_posta + "' WHERE uye_ogrencino='" + YetkiliGiris.gkadi + "'", con);
            kmd.ExecuteNonQuery();
            con.Close();
        }
    }
